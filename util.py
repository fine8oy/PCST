...
def st_prompt(data_path, data_len=0, n_var=0, data_name='temp'):
    ...
    if data_name == 'temp' or data_name == 'pm2_5':
        ...
        time_stamp = time_stamp.repeat(1, n_var, 1)
    if data_name == 'metrla':
    ...
    if data_name == 'tongzhou':
        ...
        time_stamp = time_stamp.unsqueeze(1)
        time_stamp = time_stamp.repeat(1, n_var, 1)
    spatial_prompt = torch.load(os.path.join(data_path, 'city.pt'))
    spatial_prompt = spatial_prompt.unsqueeze(0).repeat(data_len, 1, 1)
...
def split_train_val_test(x, y, data_name='temp'):
    ...
    if data_name == 'temp' or data_name == 'pm2_5':
        ...
        # 9月 微调prompt
        # 10 11 12 月 test
        x_train = torch.cat([x[:181 * 8], x[365 * 8:(365 + 182) * 8], x[(365 + 366) * 8:(365 + 366 + 181) * 8],
                             x[(365 + 366 + 365) * 8:(365 + 366 + 365 + 181) * 8]], dim=0)
        y_train = torch.cat([y[:181 * 8], y[365 * 8:(365 + 182) * 8], y[(365 + 366) * 8:(365 + 366 + 181) * 8],
                             y[(365 + 366 + 365) * 8:(365 + 366 + 365 + 181) * 8]], dim=0)

        x_val = torch.cat([x[181 * 8:(181 + 62) * 8], x[(365 + 182) * 8:(365 + 182 + 62) * 8],
                           x[(365 + 366 + 181) * 8:(365 + 366 + 181 + 62) * 8],
                           x[(365 + 366 + 365 + 181) * 8:(365 + 366 + 365 + 181 + 62) * 8]], dim=0)
        y_val = torch.cat([y[181 * 8:(181 + 62) * 8], y[(365 + 182) * 8:(365 + 182 + 62) * 8],
                           y[(365 + 366 + 181) * 8:(365 + 366 + 181 + 62) * 8],
                           y[(365 + 366 + 365 + 181) * 8:(365 + 366 + 365 + 181 + 62) * 8]], dim=0)

        x_prompt = torch.cat([x[(181 + 62) * 8:(181 + 62 + 30) * 8], x[(365 + 182 + 62) * 8:(365 + 182 + 62 + 30) * 8],
                              x[(365 + 366 + 181 + 62) * 8:(365 + 366 + 181 + 62 + 30) * 8],
                              x[(365 + 366 + 365 + 181 + 62) * 8:(365 + 366 + 365 + 181 + 62 + 30) * 8]], dim=0)
        y_prompt = torch.cat([y[(181 + 62) * 8:(181 + 62 + 30) * 8], y[(365 + 182 + 62) * 8:(365 + 182 + 62 + 30) * 8],
                              y[(365 + 366 + 181 + 62) * 8:(365 + 366 + 181 + 62 + 30) * 8],
                              y[(365 + 366 + 365 + 181 + 62) * 8:(365 + 366 + 365 + 181 + 62 + 30) * 8]], dim=0)

        x_test = torch.cat([x[(181 + 62 + 30) * 8:(181 + 62 + 30 + 92) * 8],
                            x[(365 + 182 + 62 + 30) * 8:(365 + 182 + 62 + 30 + 92) * 8],
                            x[(365 + 366 + 181 + 62 + 30) * 8:(365 + 366 + 181 + 62 + 30 + 92) * 8],
                            x[(365 + 366 + 365 + 181 + 62 + 30) * 8:]], dim=0)  # 滑窗导致数据不满
        y_test = torch.cat([y[(181 + 62 + 30) * 8:(181 + 62 + 30 + 92) * 8],
                            y[(365 + 182 + 62 + 30) * 8:(365 + 182 + 62 + 30 + 92) * 8],
                            y[(365 + 366 + 181 + 62 + 30) * 8:(365 + 366 + 181 + 62 + 30 + 92) * 8],
                            y[(365 + 366 + 365 + 181 + 62 + 30) * 8:]], dim=0)
    if data_name == 'metrla':
        # 8-16 train
        # 16-24 valid
        # 0-1 prompt
        ...
        x_train, y_train, x_val, y_val, x_prompt, y_prompt, x_test, y_test = torch.cat(x_train, dim=0), torch.cat(
            y_train, dim=0), torch.cat(x_val, dim=0), torch.cat(y_val, dim=0), torch.cat(x_prompt, dim=0), torch.cat(
            y_prompt, dim=0), torch.cat(x_test, dim=0), torch.cat(y_test, dim=0)
    if data_name == 'tongzhou':
        # 8-16 train
        # 16-24 valid
        # 0-1 prompt
        ...
        x_train, y_train, x_val, y_val, x_prompt, y_prompt, x_test, y_test = [], [], [], [], [], [], [], []
        # 5min-level采样频率，一小时12个数据点，8小时96个数据点
        for i in range(87):  # 共计87天
            x_train.append(x[i * 288 + 96:i * 288 + 192])
            y_train.append(y[i * 288 + 96:i * 288 + 192])

            if i == 86:  # 滑窗导致最后一天16-24点数据不满
                x_val.append(x[i * 288 + 192:])
                y_val.append(y[i * 288 + 192:])
            else:
                x_val.append(x[i * 288 + 192:i * 288 + 288])
...
def load_dataset(batch_size, data_name='temp', time_len=12):
    data = {}
    root_path = '/data/wangziang/ComS2T-master/data'
    data_path = os.path.join(root_path, data_name)
    stdata = torch.tensor(np.load(os.path.join(data_path, f'{data_name}.npy')).transpose())  # [time,nodes]->[nodes,time]
    print(stdata.shape)
    ...
    y_train = y_train.transpose(1, 3).squeeze()[:,:,:12]
    y_test = y_test.transpose(1, 3).squeeze()

    print(x_train.shape, x_val.shape, x_prompt.shape, x_test.shape)
    scaler = StandardScaler(mean=x_train.nanmean(), std=x_train.std())
