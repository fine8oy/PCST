...
parser.add_argument('--data', type=str, default='tongzhou', help='data path')
...
parser.add_argument('--prompt_epochs', type=int, default=20, help='')
...
parser.add_argument('--prompt_dim', type=int, default=128, help='')
...
parser.add_argument('--kl_threshold', type=float, default=1.72, help='ood threshold')
parser.add_argument('--bins', type=int, default=50, help='bins number for residuals distribution')
...
def compute_histogram(tensor, bins):
    # 计算直方图，返回每个区间的计数
    min = tensor.min().item()
    max = tensor.max().item()
    counts = torch.histc(tensor, bins=bins, min=min, max=max)
...
def compute_kl_divergence(P, Q, epsilon=1e-10):
    # 确保P和Q是有效的概率分布（非负且和为1），并添加epsilon以避免log(0)
...
def test(engine, dataloader):
    ...
    # Node involve -- 测试是将删除的节点复原
    engine.add_list = None
...
def train_prompt(engine, dataloader):
    ...
    for i in range(1, args.prompt_epochs + 1):
        ...
        for iter, (x, y) in enumerate(dataloader['train_loader'].get_iterator()):
            ...
            metric_t = engine.train_prompt_t(trainx, trainy[:, 0, :, :])
            metric_s = engine.train_prompt_s(trainx, trainy[:, 0, :, :])
            train_loss_t.append(metric_t)
            train_loss_s.append(metric_s)
            if iter % args.print_every == 0:
                log = 'Iter: {:03d}, Temporal Train Loss: {:.4f}, Spatial Train Loss: {:.4f}'
                print(log.format(iter, train_loss_t[-1], train_loss_s[-1], flush=True))

        mtrain_loss_t = np.mean(train_loss_t)
        mtrain_loss_s = np.mean(train_loss_s)
        logs = 'Epoch: {:03d}, Temporal Train Loss: {:.4f}, Spatial Train Loss: {:.4f}'
        print(logs.format(i, mtrain_loss_t, mtrain_loss_s))
    return engine
...
def main_train():
    ...
    engine = trainer(args.ablation_pmt, args.prompt_dim, scaler, args.in_dim, args.seq_length, num_nodes, args.nhid,
                     args.dropout,
    ...
    for i in range(1, args.epochs + 1):
        ...
        for iter, (x, y) in enumerate(dataloader['train_loader'].get_iterator()):
            ...
            # 训练前的模型参数
            param_bef = engine.model.state_dict()
            ...
            # autoencoder单独训练prompt
            metrics_prompt_t = engine.train_prompt_t(trainx, trainy[:, 0, :, :])
            metrics_prompt_s = engine.train_prompt_s(trainx, trainy[:, 0, :, :])

            # 训练主模型
            metrics = engine.train(trainx, trainy[:, 0, :, :])

            # 当前残差
            cur_residual = metrics[3]
            # 保存当前残差
            if iter == 0:
                residuals = cur_residual
            else:
                residuals = torch.cat((residuals, cur_residual), dim=0)
            # 残差张量转换为分布
            # 先将离散数据转换成离散分布 再计算kl
            dis_cur_residual = compute_histogram(cur_residual, args.bins)
            dis_residuals = compute_histogram(residuals, args.bins)
            kl_div = compute_kl_divergence(dis_cur_residual, dis_residuals)

            if kl_div > 0:
                # print('a new distribution, start adopting...')
                # ood -- 参数划分
                param_aft = engine.model.state_dict()
                # 找到变化最大的参数群
                instable_params, num_t = param_split(param_bef, param_aft)
                # 设置稳定参数的requires_grad为False
                for name, param in engine.model.named_parameters():
                    if name not in instable_params.keys() and ('conv' in name or 'mlp' in name):
                        param.requires_grad = False
                for param in engine.model.pemb_multi_s.parameters():
                    param.requires_grad = True
                for param in engine.model.pemb_multi_t.parameters():
                    param.requires_grad = True
                ...
                # 适应完毕，所有参数的requires_grad设置回True
                for param in engine.model.parameters():
                    param.requires_grad = True
                # print('finish adopting...')
...
def finetune_test(engine, dataloader, instable_params):
    if args.retrain:
        engine.set_model()
    ...
    val_time = []
    train_time = []
    add_list = engine.add_list
    for i in range(1, args.epochs + 1):
        train_loss = []
        train_mape = []
        train_rmse = []
        engine.add_list = add_list
        t1 = time.time()
        dataloader['train_loader'].shuffle()
        for iter, (x, y) in enumerate(dataloader['train_loader'].get_iterator()):
            trainx = torch.Tensor(x).to(device)
    ...
    engine.model.load_state_dict(
        torch.load(args.save + "_epoch_" + str(bestid + 1) + ".pth"))  # +"_"+str(round(his_loss[bestid],2))
...
def search_var_invar():
    ...
    for i in range(max(0, args.epochs - 10), args.epochs):
        model_parameters.append(
            torch.load(args.save + "_epoch_" + str(i + 1) + ".pth", map_location='cpu'))  # torch.load()加载之后的pth文件就是dict
...
def param_split(param_bef, param_aft):
    # 寻找稳定的 和 不稳定的权值
    # print('search_var_invar...')
    # 步骤1: 加载模型参数文件
    model_parameters = [param_bef, param_aft]
    keys = model_parameters[0].keys()
    # 步骤2: 计算均值和偏差程度
    variance_params = {}
    instable_params = {}
...
if __name__ == "__main__":
    ...
    engine.use_prompt = 1
    finetune_test(engine, dataloader, instable_params)
